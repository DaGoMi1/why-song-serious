...
class DeepFM(nn.Module):
    def __init__(self, feature_dims, embedding_dim, continuous_cols, hidden_layers=[128, 64], dropout=0.2):
        super().__init__()
    ...
    def forward(self, catecorical_x, continuous_x):
        embedded_list = [emb(catecorical_x[:, i]) for i, emb in enumerate(self.embeddings)]
        embeddings = torch.stack(embedded_list, dim=1)

        linear_input = torch.cat([catecorical_x.float(), continuous_x], dim=1)
